<?php 
    ...
    class lsp{
        ...
        public function login($username,$password){
            ...
            if($rows > 0){
                if(base64_decode($assoc['password']) == $password){
                    return ['response'=>'positive','alert'=>'Berhasil Login woy','level'=>$assoc['level']];
                }else{
                    return ['response'=>'negative','alert'=>'Password Salah woy'];    
                }
            }else{
                global $con; 
            $sql2 = "SELECT * FROM table_marketing WHERE username ='$username'";
            $query2 = mysqli_query($con,$sql2);
            $rows2  = mysqli_num_rows($query2);
            $assoc2 = mysqli_fetch_assoc($query2);
            if($rows2 > 0){
                if($assoc2['password'] == $password){
                    return ['response'=>'positive','alert'=>'Berhasil Login','level'=>$assoc2['level']];
                }else{
        ...
        public function loginmarketing($username,$password){
            global $con;
            
            $sql = "SELECT * FROM table_marketing WHERE username ='$username'";
            $query = mysqli_query($con,$sql);
            $rows  = mysqli_num_rows($query);
            $assoc = mysqli_fetch_assoc($query);
            if($rows > 0){
        ...
        public function AuthUserMarketing($sessionUserMarketing){
            global $con;
            $sql = "SELECT * FROM table_marketing WHERE username = '$sessionUserMarketing'";
            $query = mysqli_query($con,$sql);
            $bigData = mysqli_fetch_assoc($query);
            return $bigData;
        }
        ...
        public function sessionCheckMarketing(){
            if(!isset($_SESSION['username'])){
